...
function AprioriSet(setArray){

}
AprioriSet.prototype = new Array;
AprioriSet.prototype.getSpport = function(){
...
Apriori.prototype.countSet = function(set){
	...
	//var key = set.join('='); if(itemset[key]) return itemset[key].count;
	...
	for(var ln=1; ln <= level ; ln++){
		...
		_.each(collections, function(items){
			...
			_.each(comb, function(item){
				...
				if(!itemset[key]){
					var count = self.countSet(item)
					var support = count/total; if(support < minSup) return;
					itemset[key] = {
						list: 		item, 
						support: 	support,
						count : 	count,
